...
[at_vect, et_vect, it, OMt, omt_vect, deltaV_tot_vect, deltat2_vect] = directTransfer_opt(rrcp, rrf, vvcp, vvf);
[~, k] = min(deltaV_tot_vect);
at = at_vect(k);
et = et_vect(k);
omt = omt_vect(k);
deltaV_tot = deltaV_tot_vect(k);
deltat2 = deltat2_vect(k);
...
deltaV_tot = abs(DeltaV1) + abs(deltaV_tot)
