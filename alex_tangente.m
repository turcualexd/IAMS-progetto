...
%% Plotto grafico delle deltaV totali minime in funzione delle deltaV iniziali
figure
plot(deltaV_vect, deltaV_min_vect)
...
[~, k_temp] = min(deltaV_cp);

%% Secante
[rrcp, vvcp] = par2car(at(k_temp), et(k_temp), i_f, OMf, om_cp(k_temp), th_cp(k_temp));
[as, es, i_s, OMs, oms, th1_trasf, th2_trasf, th1_iniziale, th2_finale, deltaV_tot, deltaT_tot, rr1, rr2] = secante_ottimale(rrcp, vvcp, rrf, vvf, nan, nan, true);
deltaV_tot
deltaV_tot + vmin
...
%plotOrbit(at(k_temp), et(k_temp), i_i, OMi, omt(k_temp), 0, 2*pi, 1e-3, 'rad', 'g')
plotOrbit(at(k_temp), et(k_temp), i_f, OMf, om_cp(k_temp), 0, 2*pi, 1e-3, 'rad', 'k')